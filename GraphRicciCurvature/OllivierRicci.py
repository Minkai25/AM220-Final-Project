...
def _compute_ricci_curvature_single_edge(source, target):
    ...
    # Retrieve densities and neighbors for the source and target nodes
    x, y, neighbors_x, neighbors_y, d = _distribute_densities(source, target)
    optimal_total_cost = 0
    optimal_cost = None

    if _method == "OTD" or _method == "ATD":
        # For OTD and ATD, use the Earth Mover's Distance (EMD)
        optimal_plan = ot.emd(x, y, d)
        optimal_cost = optimal_plan * d
        optimal_total_cost = np.sum(optimal_cost)
    elif _method == "Sinkhorn":
        # For Sinkhorn, use Sinkhorn's approximation to calculate optimal transportation distance
        optimal_plan = ot.sinkhorn(x, y, d, reg=10000.0)  # regularization term might need adjustment
        optimal_cost = optimal_plan * d
        optimal_total_cost = np.sum(optimal_cost)
    elif _method == "OTDSinkhornMix":
        # Mix of OTD and Sinkhorn based on size thresholds
        if len(x) > _OTDSinkhorn_threshold and len(y) > _OTDSinkhorn_threshold:
            optimal_plan = ot.sinkhorn(x, y, d, reg=10000.0)
        else:
            optimal_plan = ot.emd(x, y, d)
        optimal_cost = optimal_plan * d
        optimal_total_cost = np.sum(optimal_cost)

    # Convert the matrix of transportation costs to a DataFrame for readability
